...
function initializeAppDashboard(adminUser) {
    ...
    let allStockProductsData = []; // Para guardar os produtos do estoque
    ...
    const renderUsersTable = users => {
        renderTable('users-table', users.sort((a,b) => (a.nome || '').localeCompare(b.nome || '')), u => {
            const roleBadges = { admin: 'bg-red-100 text-red-800', gestor: 'bg-purple-100 text-purple-800', user: 'bg-blue-100 text-blue-800', bko: 'bg-yellow-100 text-yellow-800', caixa: 'bg-green-100 text-green-800' };
            const roleNames = { admin: 'Admin', gestor: 'Gestor', user: 'Vendedor', bko: 'BKO', caixa: 'Caixa' };
            let roleKey = u.role;
            if (u.role === 'gestor' && u.isVendedor === false) {
                roleKey = 'bko';
            }
            ...
            if (u.role === 'gestor' || u.role === 'admin') {
                ...
                isVendedorToggle = `<td class="p-2 text-center"><button data-uid="${u.id}" data-is-vendedor="${!active}" class="toggle-is-vendedor-btn text-sm font-semibold py-1 px-3 rounded-full ${active ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-700'}">${active ? 'Sim' : 'Não'}</button></td>`;
            } else if (u.role === 'user') {
            ...
            return `<tr class="border-b">
                        ...
                        <td class="p-2"><span class="px-2 py-1 text-xs font-semibold rounded-full ${roleBadges[roleKey] || 'bg-gray-100'}">${roleNames[roleKey] || 'N/A'}</span></td>
                        ${isVendedorToggle}
                        <td class="p-2 flex gap-1">
                            ...
                            <button data-uid="${u.id}" data-role="caixa" class="change-role-btn text-sm ${u.role === 'caixa' ? 'bg-green-500' : 'bg-gray-300'} text-white py-1 px-2 rounded" title="Tornar Caixa">C</button>
                        </td>
    ...
    const updateInventoryDashboard = (products) => {
        if (!products) return;
        const totalValue = products.reduce((sum, p) => sum + ((p.estoqueAtual || 0) * (p.custoCompra || 0)), 0);
        document.getElementById('kpi-valor-estoque').textContent = totalValue.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
        const totalItems = products.reduce((sum, p) => sum + (p.estoqueAtual || 0), 0);
        document.getElementById('kpi-itens-estoque').textContent = totalItems;
        const lowStockProducts = products.filter(p => (p.estoqueAtual || 0) <= (p.estoqueMinimo || 0));
        document.getElementById('kpi-estoque-baixo').textContent = lowStockProducts.length;
        ...
        if (lowStockProducts.length > 0) {
            lowStockTableBody.innerHTML = lowStockProducts.map(p => `
                <tr class="border-b hover:bg-red-50">
                    ...
                    <td class="p-2 text-center font-bold text-red-600">${p.estoqueAtual || 0}</td>
                    <td class="p-2 text-center text-gray-600">${p.estoqueMinimo || 0}</td>
                </tr>
            `).join('');
        } else {
            lowStockTableBody.innerHTML = `<tr><td colspan="3" class="text-center p-4 text-gray-500">Nenhum produto com estoque baixo.</td></tr>`;
        }
    };
    ...
    const listenToAllData = () => { 
        ...
        // Listener para a coleção de estoque
        onSnapshot(query(collection(db, 'produtos_estoque'), orderBy('nome')), s => { 
            allStockProductsData = s.docs.map(d => ({id: d.id, ...d.data()}));
            updateInventoryDashboard(allStockProductsData);
        });
    };
    ...
    const setupEventListeners = () => {
        ...
        // Listeners de Produtos (Serviços)
